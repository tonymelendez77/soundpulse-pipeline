...
def load_training_data(client: bigquery.Client, region: str) -> pd.DataFrame:
    """Load weekly_features for a specific region.
    Mood pct columns (euphoric_pct etc.) are already in weekly_features from Layer 3.
    """
    query = f"""
        SELECT
...
def fetch_latest_emotions(client: bigquery.Client, region: str) -> tuple[dict, dict]:
    """Return (emotion_dict, mood_pct_dict) for the most recent week of this region."""
    query = f"""
        SELECT
            avg_fear, avg_anger, avg_joy, avg_sadness,
            avg_surprise, avg_disgust, avg_neutral,
...
def fetch_month_emotions(client: bigquery.Client, region: str, year: int, month: int) -> tuple[dict, dict]:
    """Return avg (emotion_dict, mood_pct_dict) across all weeks of this month for this region."""
    query = f"""
        SELECT
...
def train_region(client, region, now_ts, today_d, this_monday, this_month1):
    """Train one XGBoost model for `region`. Returns (pred_rows, shap_rows) or ([], [])."""
    pred_rows = []
    shap_rows = []
    ...
    if n_classes < 2:
        ...
        logger.warning(f"  [{region}] Only 1 class ('{dominant_mood}') — 1-class fallback")
        try:
...
def main():
    ...
    # 0. Outcome validation — fills in `correct` for past predictions
    ...
    if not all_pred_rows:
        logger.error("No predictions produced for any region. Run Layers 1–3 first.")
        return

    # Write both tables at the end (single truncate+load each)
    ...
    logger.info(f"  Regions trained        : {len(REGIONS)}")
    logger.info(f"  Total pred rows        : {len(all_pred_rows)}")
    logger.info(f"  Forward-inference rows : {len(fwd_rows)} ({len(REGIONS)} regions × 3 periods)")
    logger.info(f"  SHAP rows              : {len(all_shap_rows)}")
